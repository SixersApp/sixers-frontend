...
class DraftStateService {
    ...
    channel.onPostgresChanges(
      ...
      callback: (payload) {
        if (payload.newRecord['league_id'] == leagueId) {
          logDebug('RT ► ${payload.eventType} pick# ${payload.newRecord['current_pick_number']}');
  ...
  Future<DraftState?> fetch(String leagueId) async {
    try {
      final res = await _c.from('league_draft_state').select('*').eq('league_id', leagueId).maybeSingle();
