...
import 'package:sixers/views/components/draft_widgets/draft_tab_draft_widgets/position_filter_button.dart';
...
class DraftTabDraft extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final AsyncValue<List<RealTeam>> realTeamsA = ref.watch(realTeamsProvider(tournamentId: tournamentId));

    final role = _roleValue(selectedFilter);
    final filteredPlayers = role == null ? availablePlayers : availablePlayers.where((p) => p.role == role).toList();
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text('Your Pick', style: Theme.of(context).textTheme.titleLarge!.copyWith(color: AppColors.black700)),
          ),
          // Position filter
          Padding(
            padding: const EdgeInsets.only(bottom: 10),
            child: PositionFilterButton(selected: selectedFilter, onChanged: onFilterChanged),
          ),
          // Header row
          Padding(
            padding: const EdgeInsets.only(bottom: 10),
