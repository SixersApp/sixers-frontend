...
import 'package:sixers/views/components/draft_widgets/draft_app_bar.dart';
import 'package:sixers/views/components/draft_tabs/draft_tab_board.dart';
import 'package:sixers/views/components/draft_tabs/draft_tab_draft.dart';
import 'package:sixers/views/components/draft_tabs/draft_tab_roster.dart';
import 'package:sixers/views/components/draft_widgets/drafted_pick_card.dart';
...
class _DraftLobbyState extends ConsumerState<DraftLobby> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final AsyncValue<DraftState?> stateAv = ref.watch(draftStateStreamProvider(widget.league.id));
    ...
    return stateAv.when(
      ...
      data: (state) {
        ...
        final teamsAv = ref.watch(fantasyTeamsProvider(leagueId: widget.league.id));
        final playersAv = ref.watch(allPlayersProvider(widget.league.tournamentId));

        final initialLoading = [picksAv, teamsAv, playersAv].any((a) => a.isLoading && !a.hasValue);
        if (initialLoading) {
                  ...
                  child: Column(
                    children: [
                      SizedBox(
                        ...
                        width: 344,
                        child: Container(
                          decoration: BoxDecoration(color: _black100(context), borderRadius: BorderRadius.circular(90)),
                          child: TabBar(
                      ...
                      Expanded(
                        child: TabBarView(
                          ...
                          children: [
                            ...
                            DraftTabBoard(leagueId: widget.league.id, tournamentId: widget.league.tournamentId),
                            DraftTabRoster(
                              myTeamId: (myTeam as dynamic)?.id ?? '',
                              allPicks: picks,
